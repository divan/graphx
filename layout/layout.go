...
func NewAuto(g *graph.Graph) *Layout {
	worldSize := 5000 // TODO: this should be synced/communicated to with frontend somehow
	optimalEdge := estimateOptimalEdge(worldSize, len(g.Links()))
	fmt.Println("Optimal edge:", optimalEdge)
	repelling := NewGravityForce(-200.0, BarneHutMethod)
...
func estimateOptimalEdge(width, links int) float64 {
	k := 0.2
	return k * math.Sqrt(float64(width*width)/float64(links))
}
